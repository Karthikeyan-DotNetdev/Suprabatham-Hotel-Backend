...
{
    public class KOTPrinterSettingService
    {
        private readonly string _connectionString;

        public KOTPrinterSettingService(IConfiguration configuration)
        {
        ...
        public async Task<List<KOTPrinterSetting>> GetKOTPrinterSettingsByBranchAsync(string branchCode)
        {
            using var conn = new SqlConnection(_connectionString);
            using var cmd = new SqlCommand("sp_GetKOTPrinterSettings", conn) { CommandType = CommandType.StoredProcedure };
            cmd.Parameters.AddWithValue("@BranchCode", branchCode);
            await conn.OpenAsync();
            using var reader = await cmd.ExecuteReaderAsync();
            var list = new List<KOTPrinterSetting>();
            while (await reader.ReadAsync())
            {
                list.Add(new KOTPrinterSetting
                {
                    Branch_Code = reader.GetString(reader.GetOrdinal("Branch_Code")),
                    Kitchen_Code = reader.GetString(reader.GetOrdinal("Kitchen_Code")),
                    Kitchen_Name = reader.GetString(reader.GetOrdinal("Kitchen_Name")),
        ...
        public async Task SaveKOTPrinterSettingAsync(KOTPrinterSetting s, string updatedBy)
        {
            using var conn = new SqlConnection(_connectionString);
            using var cmd = new SqlCommand("sp_SaveKOTPrinterSetting", conn) { CommandType = CommandType.StoredProcedure };
            cmd.Parameters.AddWithValue("@BranchCode", s.Branch_Code);
            cmd.Parameters.AddWithValue("@KitchenCode", s.Kitchen_Code);
            cmd.Parameters.AddWithValue("@KitchenName", s.Kitchen_Name);
